...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($user_name) && !empty($password) && !is_numeric($user_name)) {
        //save to database
        $user_id = random_num(20); // create random number with max value is 20
        $query = $db->prepare("INSERT INTO users SET user_name = :user_name,password = :password,user_id = :user_id");
        $insert = $query->execute(array("user_name" => $user_name,"password" => $password,"user_id" => $user_id));
